...
//		NSLog("Database path: \(path)")
    ...
    func dropTable(table:String)->CInt {
        var result:CInt = 0;
        if(!table.isEmpty){
            result = self.execute(sql:"drop TABLE if exists \(table)", parameters: nil);
        }
        return result
    }
    ...
    func getTableName()->[String] {
        ...
        let data = self.query(sql:"SELECT name FROM sqlite_master WHERE type='table' order by name", parameters: nil);
        for item in data{
            tableNames.append(item["name"] as! String);
        }
        return tableNames
    }
    ...
    func dropAllTable()->CInt {
        ...
        for tableName in tableNames{
            if(tableName.lowercased() != "sqlite_sequence"){
                result += self.execute(sql:"drop TABLE if exists \(tableName)", parameters: nil);
            }
    ...
    func clearTable(tableName:String)->CInt {
        var result:CInt = 0;
        if(tableName != ""){
            result = self.execute(sql:"delete from \(tableName)", parameters: nil);
        }
        return result;
    }
    ...
    func clearAllTable()->CInt {
        ...
        for tableName in tableNames{
            if(tableName != ""){
                result += self.clearTable(tableName: tableName);
            }
        }
        return result;
    }
