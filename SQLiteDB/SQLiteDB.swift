...
class SQLiteDB {
    ...
    ///执行带有参数的sql语句
    /// :insert: 返回值:主键为integer就返回主键id，主键为char则返回系统自增id
    /// :delete: 返回值为删除的条数
    /// :update: 返回值为修改的条数
    ...
    ///如果存在就删除表
    func dropTable(table:String)->CInt {
        var result:CInt = 0;
        if(!table.isEmpty){
            result = self.execute("drop TABLE if exists \(table)", parameters: nil);
        }
        return result
    }
    ...
    func getTableName()->[String] {
        ...
        let data = self.query("SELECT name FROM sqlite_master WHERE type='table' order by name", parameters: nil);
        for item in data{
            tableNames.append(item.data["name"]!.asString());
        }
        return tableNames
    }
    
    ///删除所有表
    func dropAllTable()->CInt {
        var result:CInt = 0;
        let tableNames = self.getTableName();
        for tableName in tableNames{
            if(tableName.lowercaseString != "sqlite_sequence"){
                result += self.execute("drop TABLE if exists \(tableName)", parameters: nil);
            }
    ...
    ///清空表的数据
    func clearTable(tableName:String)->CInt {
        var result:CInt = 0;
        if(tableName != ""){
            result = self.execute("delete from \(tableName)", parameters: nil);
        }
        return result;
    }
    
    ///清空所有表的数据
    func clearAllTable()->CInt {
        var result:CInt = 0;
        let tableNames = self.getTableName();
        for tableName in tableNames{
            if(tableName != ""){
                result += self.clearTable(tableName);
            }
        }
        return result;
    }
